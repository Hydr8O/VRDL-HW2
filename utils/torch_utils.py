    ...
    if cuda:
        ...
        if (
            ...
            assert batch_size % ng == 0, (
                "batch-size %g not multiple of GPU count %g"
                % (batch_size, ng)
            )
        ...
        for i in range(0, ng):
            ...
            print(
                "%sdevice%g \
                _CudaDeviceProperties(name='%s', total_memory=%dMB)"
...
class ModelEMA:
    ...
    Keep a moving average of everything in the model
    state_dict (parameters and buffers).
    This is intended to allow functionality like
    https://www.tensorflow.org/api_docs/python/tf/train/ExponentialMovingAverage
    A smoothed version of the weights is necessary
    for some training schemes to perform well.
    This class is sensitive where it is initialized in
    the sequence of model init,
