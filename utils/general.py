...
def torch_distributed_zero_first(local_rank: int):
    """
    Decorator to make all processes in distributed
    training wait for each local_master to do something.
    """
...
def check_git_status():
    ...
    if platform in ["linux", "darwin"] and not os.path.isfile(
        "/.dockerenv"
    ):
...
def check_img_size(img_size, s=32):
    ...
    if new_size != img_size:
        print(
            "WARNING: --img-size %g must be multiple of max stride %g, \
            updating to %g" % (img_size, s, new_size)
        )
    return new_size
...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    if bpr < 0.98:  # threshold to recompute
        print(
            ". Attempting to generate improved anchors, please wait..."
            % bpr
        )
        ...
        if new_bpr > bpr:  # replace anchors
            ...
            print(
                "Original anchors better than new anchors. \
...
def labels_to_class_weights(labels, nc=80):
    ...
    labels = np.concatenate(
        labels, 0
    )  # labels.shape = (866643, 5) for COCO
...
def compute_loss(
    ...
    tcls, tbox, indices, anchors, _ = build_targets(
        p, targets, model, test
    )  # targets
    h = model.hyp  # hyperparameters

    # Define criteria
    BCEcls = nn.BCEWithLogitsLoss(
        pos_weight=torch.Tensor([h["cls_pw"]])
    ).to(device)
    BCEobj = nn.BCEWithLogitsLoss(
        pos_weight=torch.Tensor([h["obj_pw"]])
    ).to(device)
    ...
    balance = (
        [4.0, 1.0, 0.4] if np == 3 else [4.0, 1.0, 0.4, 0.1]
    )  # P3-5 or P3-6
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            giou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=False)
...
def build_targets(p, targets, model, test=False):
    nt = targets.shape[0]  # number of anchors, targets
    all_kd_masks = []
    tcls, tbox, indices, anch = [], [], [], []
    gain = torch.ones(
        6, device=targets.device
    ...
    for i, jj in enumerate(
        ...
        box_predictions = []
        box_predictions.append(p[i][..., :4])
        nB = box_predictions[0].size(0)
        nA = box_predictions[0].size(1)
        nG = box_predictions[0].size(2)

        tbox.append(torch.cat((gxy - gij, gwh), 1))
        iou_scores = torch.zeros(nB, nA, nG, nG, dtype=torch.float32).to(
            tbox[0].device
        )

        if not test:
            iou_scores[b, a, gj, gi] = bbox_iou_another(
                box_predictions[0][b, a, gj, gi], tbox[i], x1y1x2y2=False
            )
            threshold = 0.5  # Threshold for IOU mask
            kd_mask = torch.zeros(nB, nG, nG, dtype=torch.float32).to(
                targets.device
            )
            for i in range(nB):
...
def compute_loss_KD(
    feature_adaptation_layer, model, targets, output_t, feature_s, feature_t
):
    lambda_feature = 0.001

    lfeature = 0
    tcls, tbox, indices, anchor_vec, kd_masks = build_targets(
    ...
    lfeature += (
        torch.pow(
            feature_t[2] - feature_adaptation_layer(feature_s[0], 1), 2
        )
        * kd_mask_3
    ).sum() / norms1
    lfeature += (
        torch.pow(
            feature_t[1] - feature_adaptation_layer(feature_s[1], 2), 2
        )
        * kd_mask_2
    ).sum() / norms2
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if classes:
            x = x[
                (x[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)
            ]
...
def coco_single_class_labels(
    ...
    for file in tqdm(sorted(glob.glob("%s/*.*" % path))):
        ...
        if any(i):
            img_file = file.replace("labels", "images").replace(
                "txt", "jpg"
            )
            labels[:, 0] = 0  # reset class to 0
            with open(
                "new/images.txt", "a"
            ) as f:  # add image to dataset list
                f.write(img_file + "\n")
            with open(
                "new/labels/" + Path(file).name, "a"
            ) as f:  # write label
                for l in labels[i]:
...
def kmean_anchors(
    ...
    def print_results(k):
        ...
        print(
            "n=%g, img_size=%s, metric_all=%.3f/%.3f-mean/ \
            best, past_thr=%.3f-mean: "
    ...
    # Get label wh
    shapes = (
        img_size * dataset.shapes / dataset.shapes.max(1, keepdims=True)
    )
    wh0 = np.concatenate(
    ...
    for _ in pbar:
        ...
        while (
            ...
            v = (
                (npr.random(sh) < mp) * npr.random() * npr.randn(*sh) * s
                + 1
            ).clip(0.3, 3.0)
...
def print_mutation(hyp, results, yaml_file="hyp_evolved.yaml", bucket=""):
    ...
    with open("evolve.txt", "a") as f:  # append result
        f.write(c + b + "\n")
    x = np.unique(
        np.loadtxt("evolve.txt", ndmin=2), axis=0
    )  # load unique rows
    ...
    if bucket:
        os.system(
            "gsutil cp evolve.txt gs://%s" % bucket
        )  # upload evolve.txt
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                cutout = im0[i][
                    int(a[1]): int(a[3]), int(a[0]): int(a[2])
                ]
    ...
    if len(d):
        n = (
            max(
                [
                    int(
                        x[len(dir): x.find("_") if "_" in x else None]
                    )
                    for x in d
                ]
            )
