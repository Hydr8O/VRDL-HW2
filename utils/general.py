...
def compute_loss(p, targets, model, test=True):  # predictions, targets, model
    device = targets.device
    lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
    tcls, tbox, indices, anchors, _ = build_targets(p, targets, model, test)  # targets
...
def build_targets(p, targets, model, test=False):
    ...
    for i, jj in enumerate(model.module.yolo_layers if multi_gpu else model.yolo_layers):
        ...
        iou_scores = torch.zeros(
                    nB, nA, nG, nG, dtype=torch.float32).to(tbox[0].device)
        
        if not test:
            iou_scores[b, a, gj, gi] = (bbox_iou_another(box_predictions[0][b, a, gj,
                                                                            gi], tbox[i], x1y1x2y2=False))
            threshold = 0.5  # Threshold for IOU mask
            kd_mask = torch.zeros(
                nB, nG, nG, dtype=torch.float32).to(targets.device)
            for i in range(nB):
...
def compute_loss_KD(feature_adaptation_layer, model, targets, output_t, feature_s, feature_t):
    ...
    lfeature += (torch.pow(feature_t[2] - feature_adaptation_layer(feature_s[0], 1), 2)
                 * kd_mask_3).sum() / norms1
