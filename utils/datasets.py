...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640):
        ...
        if any(videos):
            ...
            self.cap = None
        assert self.nf > 0, 'No images or videos found in %s. Supported \
        formats are:\nimages: %s\nvideos: %s' % \
    ...
    def __init__(
        ...
        try:
            ...
            self.img_files = sorted([
                x.replace('/', os.sep)
                for x in f if os.path.splitext(x)[-1].lower() in img_formats
            ])
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                assert (l[:, 1:] <= 1).all(
                ), 'non-normalized or out of b \
                ounds coordinate labels: %s' % file
            ...
            pbar.desc = 'Scanning labels %s (%g found, %g missing, \
             %g empty, %g duplicate, for %g images)' % (
