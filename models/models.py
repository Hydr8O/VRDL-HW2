...
class FeatureAdaptation(nn.Module):
    def __init__(self):
        ...
        self.adaptation_layer1 = nn.Sequential(
            nn.Conv2d(512, 315, kernel_size=3, padding=1), nn.ReLU())
        self.adaptation_layer2 = nn.Sequential(
            nn.Conv2d(256, 374, kernel_size=3, padding=1), nn.ReLU())
        self.adaptation_layer3 = nn.Sequential(
            nn.Conv2d(315, 315, kernel_size=3, padding=1), nn.ReLU())
...
def get_yolo_layers(model):
    # [89, 101, 113]
    return [
        i for i, m in enumerate(model.module_list) if m
        .__class__
        .__name__ == 'YOLOLayer'
    ]
...
def load_darknet_weights(self, weights, cutoff=-1):
    ...
    if file == 'darknet53.conv.74':
        ...
        cutoff = 15
    elif file == 'yolov4-csp.conv.142':
        cutoff = 142
    elif file == 'yolov4x-mish.conv.166':
        cutoff = 166
...
def attempt_download(weights):
    ...
    msg = weights + ' missing, try downloading\
     from https://drive.google.com/open?id=1LezFG5g3BCW6iYaV89B2i64cqEUZD7e0'
    ...
    if len(weights) > 0 and not os.path.isfile(weights):
        ...
        path_existence = os.path.exists(weights)
        if not (
            r == 0 and path_existence and os.path.getsize(weights) > 1E6
        ):
