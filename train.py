...
def train(hyp, opt, device, tb_writer=None):
    ...
    log_dir = (
        Path(tb_writer.log_dir)
        if tb_writer
        else Path(opt.logdir) / "evolve"
    )  # logging directory
    ...
    if opt.t_cfg:
        ...
        if opt.t_weights.endswith(".pt"):
            ...
            ckpt = torch.load(
                opt.t_weights, map_location=device
            )  # load checkpoint
            state_dict = {
                k: v
                for k, v in ckpt["model"].items()
                if teacher_model.state_dict()[k].numel() == v.numel()
            }
    ...
    lf = (
        lambda x: (((1 + math.cos(x * math.pi / epochs)) / 2) ** 1.0) * 0.8
        + 0.2
    )  # cosine
        ...
        for (
            ...
            with amp.autocast(enabled=cuda):
                ...
                if opt.t_cfg:
                    ...
                    if opt.KD:
                        kd_loss = compute_loss_KD(
                            feature_adaptation_layer,
                            teacher_model,
                            targets.to(device),
                            output_t,
                            feature_s,
    ...
    if rank in [-1, 0]:
        # Strip optimizers
        n = (
            "_" if len(opt.name) and not opt.name.isnumeric() else ""
        ) + opt.name
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--logdir", type=str, default="runs/", help="logging directory"
    )
    parser.add_argument("--KD", action="store_true")
    parser.add_argument(
        "--t_cfg", type=str, default="", help="model.yaml path"
    )
    parser.add_argument(
        "--t_weights", type=str, default="", help="model.yaml path"
    )
    ...
    if opt.resume:
        ...
        opt.weights = (
            last if opt.resume and not opt.weights else opt.weights
        )
    if opt.local_rank == -1 or (
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            print(
                ...
                --logdir %s", view at http://localhost:6006/'
                % opt.logdir
            )
