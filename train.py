...
from utils.general import (
    ...
    get_latest_run, check_git_status, check_file, increment_dir, print_mutation, plot_evolution, compute_loss_KD)
from utils.google_utils import attempt_download
from utils.torch_utils import init_seeds, ModelEMA, select_device, intersect_dicts
from utils.prune_utils import parse_module_defs, BNOptimizer, gather_bn_weights
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if opt.t_cfg:
        print(opt.t_cfg, opt.t_weights)
        teacher_model = Darknet(opt.t_cfg).to(device)
        if opt.t_weights.endswith('.pt'):
            with torch_distributed_zero_first(rank):
                attempt_download(opt.t_weights)  # download if not found locally
            ckpt = torch.load(opt.t_weights, map_location=device)  # load checkpoint
            state_dict = {k: v for k, v in ckpt['model'].items() if teacher_model.state_dict()[k].numel() == v.numel()}
            teacher_model.load_state_dict(state_dict, strict=False)
            print('Transferred %g/%g items from %s' % (len(state_dict), len(teacher_model.state_dict()), opt.t_weights))
        else:
            load_darknet_weights(teacher_model, opt.t_weights)
    ...
    feature_adaptation_layer = FeatureAdaptation().to(device)
    # Optimizer
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                kd_loss = 0
                if opt.t_cfg:
                    _, output_t, feature_t = teacher_model(imgs)
                    if opt.KD:
                        kd_loss = compute_loss_KD(feature_adaptation_layer, teacher_model, targets.to(device), output_t, feature_s, feature_t)
            ...
            if hasattr(model, 'module'):
                if opt.prune != -1:
                    BNOptimizer.updateBN(
                        sr_flag, model.module.module_list, opt.s, prune_idx)
            else:
                if opt.prune != -1:
                    BNOptimizer.updateBN(
                        sr_flag, model.module_list, opt.s, prune_idx)
                ...
                if opt.prune != -1:
                    if hasattr(model, 'module'):
                        bn_weights = gather_bn_weights(
                            model.module.module_list, [idx])
                    else:
                        bn_weights = gather_bn_weights(model.module_list, [idx])
                    tb_writer.add_histogram(
                        'bn_weights/hist', bn_weights.numpy(), epoch, bins='doane')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--t_cfg', type=str, default='', help='model.yaml path')
    parser.add_argument('--t_weights', type=str, default='', help='model.yaml path')
