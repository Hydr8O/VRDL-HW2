...
from utils.general import (
    coco80_to_coco91_class, check_file, check_img_size, compute_loss,
    non_max_suppression,
...
def test(data,
    ...
    for batch_i, (
        ...
        with torch.no_grad():
            ...
            inf_out, train_out, _ = model(img, augment=augment)
    ...
    if not training:
        print(
            'Speed: %.1f/%.1f/%.1f ms inference/NMS/total \
            per %gx%g image at batch-size %g' % t
        )
    ...
    if save_json and len(jdict):
        f = 'detections_val2017_%s_results.json' % \
            (weights.split(os.sep)[-1].replace('.pt', '')
             if isinstance(weights, str) else '')  # filename
